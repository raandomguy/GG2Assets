::PEA <-
{
			...
			switch (params.wave_index)
			{
				...
				case 4:
				{
					if (secretwave_unlocked)
					{
						...
						NetProps.SetPropInt(objective_resource_entity, "m_nMannVsMachineWaveEnemyCount", 13)
					}
				...
				foreach (player in GetAllPlayers(2))
				{
					if (TraceLine(self.EyePosition(), player.EyePosition(), self) == 1.0)
					{
						if (!("vistip_medicshotgun" in player.GetScriptScope()))
						{
							...
							{
								id = player.entindex()
								text = "Giant Shotgun Medics steal\nhealth from their patients!"
	...
	SamuraiSoldier_Think = function()
	{
		...
		if (dummymodel != null)
		{
			if (dummymodel.GetCycle() >= 0.8)
			{
				...
				for (local ent; ent = Entities.FindByName(ent, "samurai_soldier_sfx"); ) ent.Kill()
		...
		if (jumping && self.GetAbsVelocity().z <= 0 && !in_cooldown)
		{
			...
			SpawnEntityFromTable("info_particle_system", { targetname = "samurai_soldier_sfx", origin = self.EyePosition() + Vector(RandomInt(-150, 150), RandomInt(-150, 150), RandomInt(-150, 150)), effect_name = "eyeboss_team_blue", start_active = 1, flag_as_weather = 0 } )
			SpawnEntityFromTable("info_particle_system", { targetname = "samurai_soldier_sfx", origin = self.EyePosition() + Vector(RandomInt(-150, 150), RandomInt(-150, 150), RandomInt(-150, 150)), effect_name = "eyeboss_team_blue", start_active = 1, flag_as_weather = 0 } )
			SpawnEntityFromTable("info_particle_system", { targetname = "samurai_soldier_sfx", origin = self.EyePosition() + Vector(RandomInt(-150, 150), RandomInt(-150, 150), RandomInt(-150, 150)), effect_name = "eyeboss_team_blue", start_active = 1, flag_as_weather = 0 } )
			SpawnEntityFromTable("info_particle_system", { targetname = "samurai_soldier_sfx", origin = self.EyePosition() + Vector(RandomInt(-150, 150), RandomInt(-150, 150), RandomInt(-150, 150)), effect_name = "eyeboss_team_blue", start_active = 1, flag_as_weather = 0 } )
			SpawnEntityFromTable("info_particle_system", { targetname = "samurai_soldier_sfx", origin = self.EyePosition() + Vector(RandomInt(-150, 150), RandomInt(-150, 150), RandomInt(-150, 150)), effect_name = "eyeboss_team_blue", start_active = 1, flag_as_weather = 0 } )
	...
	TunnelSpawnFix_Think = function()
	{
		...
		foreach (player in GetAllPlayers(3))
		{
			...
			{
				player.Teleport(true, player.GetOrigin() + Vector(0, 64, 8), false, QAngle(), false, Vector())
			}
		}
	}
...
switch (Wave)
{
	...
	case 5:
	{
		...
		{
			DisableSpawn("secretwave")

			UnhideIcon("hatch_nys", 9)
			UnhideIcon("pyro_giant", 25)
			UnhideIcon("medic_giant", 9)
			UnhideIcon("scout_giant_fast", 9)
			// SetTotalEnemyCount(13)
		}
